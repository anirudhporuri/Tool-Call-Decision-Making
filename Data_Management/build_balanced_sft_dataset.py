...
def canonicalize_record(tools, messages, source_split, behavior_class):
    return {
        "tools": tools,
        "messages": messages,
...
def build_pref_tool_pool(dataset):
    pool = []
    for row in dataset:
...
def sample_examples(examples, k, rng, label):
    if len(examples) < k:

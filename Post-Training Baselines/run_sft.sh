...
python3 train_sft_lora.py   --model_name_or_path "${MODEL_NAME_OR_PATH}"   --model_family "${MODEL_FAMILY}"   --output_dir "${OUTPUT_DIR}"   --dtype "${DTYPE}"   --num_train_epochs "${EPOCHS}"   --learning_rate "${LR}"   --per_device_train_batch_size "${TRAIN_BS}"   --per_device_eval_batch_size "${EVAL_BS}"   --gradient_accumulation_steps "${GRAD_ACCUM}"   --max_length "${MAX_LENGTH}"   --val_size "${VAL_SIZE}"   --report_to "${REPORT_TO}"   "${EXTRA_ARGS[@]}"
